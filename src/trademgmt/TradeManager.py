...
class TradeManager:
            ...
            if trade.tradeState == TradeState.ACTIVE:
                ...
                if trade.intradaySquareOffTimestamp != None:
                    ...
                    if nowEpoch >= trade.intradaySquareOffTimestamp:
                        '''We are checking Traget order entry before we are calling SQUAREOFF
                           becasue for startegies which dont have SL order at the bigining will have 
                           trade.target = 0 which make the TradeManager.trackTargetOrder to return without updating
                           Trade State to COMPLETE'''
                        if trade.targetOrder == None:
                            ...
                            TradeManager.setTradeToCompleted(trade, exit, TradeExitReason.SQUARE_OFF)
    ...
    def placeSLOrder(trade):
        ...
        try:
            ...
            return False
        logging.info('TradeManager: Successfully placed SL order %s for tradeID %s', trade.slOrder.orderId,
                     trade.tradeID)
        return True
